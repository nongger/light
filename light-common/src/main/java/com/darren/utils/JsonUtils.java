...
public class JsonUtils {
     ...
     *
     * @param suppressException: false - RunTimeException out,
    ...
    public static <T> T fromJsonObject(String json, Class<T> classOfT, boolean suppressException) {
        try {
            ...
            if (suppressException) {
                log.warn("[fromJsonObject] fail to parse string to json object, string = {}, "
                        + "just suppressException and return null, exception is: ", json, e);
                return null;
            } else {
                log.error("[fromJsonObject] fail to parse string to json object, string = {},  exception is: ", json, e);
    ...
    public static <T> List<T> fromJsonArray(String json, Class<T> classOfT, boolean suppressException) {
        try {
            ...
            if (suppressException) {
                log.warn("[fromJsonArray] fail to parse string to json array, string = {}, "
                        + "just suppressException and return null, exception is: ", json, e);
                return null;
            } else {
                log.error("[fromJsonArray] fail to parse string to json array, string = {}, exception is: ", json, e);
    ...
    public static JsonElement parseJson(String json, boolean isLenient, boolean suppressException) {
        ...
        try {
            ...
            if (isLenient) {
                log.warn("[parseJson] met malformed json: {}, use lenient mode to parse again, exception is: ", json, e);
                JsonReader reader = new JsonReader(new StringReader(json));
                reader.setLenient(true);
                return JsonParser.parseReader(reader);
            } else if (suppressException) {
                log.warn("[parseJson] met malformed json: {}, just stress exception and return null, exception is: ", json, e);
                return null;
            } else {
                log.error("[parseJson] met malformed json: {}, exception is: ", json, e);
    ...
    private static boolean checkJsonObjectBySpec(JsonObject jsonObj, JsonArray specs, String jsonPath) {
        ...
        for (int i = 0; i < specs.size(); i++) {
            ...
            if (!s.isJsonObject()) {
                log.warn("[checkJsonObjectBySpec] the spec's format is fucking wrong !");
                return false;
            }
            ...
            if (jsonObj.has(name)) {
                ...
                log.warn("[checkJsonObjectBySpec] missing path: {}", localJsonPath);
                return false;
            }
        }
        ...
        switch (type) {
            case "number":
                if (!element.isJsonPrimitive() || !element.getAsJsonPrimitive().isNumber()) {
                    log.warn("[checkJsonElementBySpec] the element is not a number, @{}", jsonPath);
                    return false;
                }
                break;
            case "string":
                if (!element.isJsonPrimitive() || !element.getAsJsonPrimitive().isString()) {
                    log.warn("[checkJsonElementBySpec] the element is not a string, @{}", jsonPath);
                    return false;
                }
                break;
            case "boolean":
                if (!element.isJsonPrimitive() || !element.getAsJsonPrimitive().isBoolean()) {
                    log.warn("[checkJsonElementBySpec] the element is not a boolean, @{}", jsonPath);
                    return false;
                }
                break;
            case "object":
                if (element.isJsonNull()) {
                    if (isJsonElementRequired(spec)) {
                        log.warn("[checkJsonElementBySpec] the element is null, but is required, @{}", jsonPath);
                        return false;
                    }
                    ...
                    log.warn("[checkJsonElementBySpec] the element is not an object, @{}", jsonPath);
                    return false;
                }
                break;
            case "array":
                ...
                if (element.isJsonArray()) {
                    ...
                    log.warn("[checkJsonElementBySpec] the element is not an array, @{}", jsonPath);
                    return false;
                }
                break;
            default:
                log.warn("[checkJsonElementBySpec] the spec's format is fucking wrong, unkonwn type {}", type);
                return false;
        }
        return true;
    }
    ...
    private static boolean isJsonElementRequired(JsonObject spec) {
        ...
        if (spec.has("required")) {
            if (spec.get("required").isJsonPrimitive()
                ...
                log.warn("[isJsonElementRequired] the spec's format fucking wrong, required should be boolean");
    ...
    public static int fromJsonPath2ArrayIndex(String path) {
        Matcher m = Pattern.compile("^__(\\d+)__$").matcher(path);
        if (m.find()) {
